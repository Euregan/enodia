...
const fieldsToQuery = () =>
  [
    ...
    "      .map((field) => {",
    "        const key: Exclude<",
    "          KeysOfUnion<Exclude<Query, string>>,",
    "          '$args'",
    "        > = Object.keys(field).filter((key) => key !== '$args')[0];",
    "",
    "        let queryField = key",
    "",
    "        if ('$args' in field) {",
    ...
    "        }",
    "",
    "        queryField += ` ${fieldsToQuery(field[key], path.concat(key))}`",
    "",
    "        return queryField;",
    "      })",
    "      .join('\\n')}\\n}`;",
...
export const resultsToArgs = () =>
  [
    "const resultsToArgs = <Q extends keyof typeof queryToGqlTypes>(query: Q, returns: Fields, path: Array<string> = []) =>",
    "  returns",
    "    ? (",
    ...
    "              .map(",
    "                (arg) =>",
    "                  `$${path.concat([key, arg]).join('_')}: ${queryToGqlTypes[query][key].$args[arg]}`",
    "              )",
    "              .join('\\n');",
    "          }",
    "",
    "          args += resultsToArgs(queryToGqlTypes[query][key].query, field[key], path.concat(key))",
    "",
    "          return args;",
    "        })",
    "        .join('\\n')",
...
const returnsToVariables = () =>
  [
    ...
    "          }",
    "",
    "          args = {...args, ...returnsToVariables(field[key], path.concat(key))}",
    "",
    "          return args;",
    "        }))",
    "    : {};",
